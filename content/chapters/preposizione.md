---
title: "La preposizione"
weight: 130
---

## Le preposizioni semplici

| | Uso principale | Esempio |
|---|---|---|
| **di** | possesso, materia, argomento, paragone | il libro **di** Maria, un tavolo **di** legno, parlare **di** politica, più alto **di** te |
| **a** | luogo (stato/moto a), tempo, termine indiretto | abito **a** Roma, vado **a** casa, alle tre, do il libro **a** Marco |
| **da** | provenienza, agente, presso, da...a, caratteristica | vengo **da** Milano, scritto **da** Dante, vado **dal** medico, **da** qui **a** lì, una ragazza **dagli** occhi verdi |
| **in** | luogo (stato/moto in), tempo, mezzo | vivo **in** Italia, vado **in** ufficio, **in** estate, viaggio **in** treno |
| **con** | compagnia, mezzo, modo | esco **con** gli amici, taglio **con** il coltello, parla **con** calma |
| **su** | luogo (sopra), argomento approssimativo | il libro è **sul** tavolo, un saggio **su** Dante |
| **per** | scopo, destinazione, durata, causa, mezzo | studio **per** imparare, parto **per** Roma, **per** due ore, **per** questo motivo |
| **tra / fra** | tempo futuro, posizione intermedia (le due forme sono equivalenti, si sceglie per evitare la ripetizione del suono) | **tra** un'ora, **fra** i libri |

Le preposizioni **di, a, da, in, su** si fondono con l'articolo determinativo che segue (**preposizioni articolate**) — vedi *L'articolo* per la tabella completa.

## Locuzioni preposizionali

Gruppi di parole che funzionano come una preposizione, quasi sempre terminanti in **di** o **a**:

*davanti a, dietro a, vicino a, lontano da, invece di, prima di, dopo di, insieme a, a causa di, secondo, durante, nonostante*

## Preposizione + infinito dopo un verbo

Molti verbi reggono un infinito attraverso una preposizione fissa, da imparare verbo per verbo — non esiste una regola generale:

| Verbo + **a** | Verbo + **di** | Verbo senza preposizione |
|---|---|---|
| cominciare **a**, iniziare **a** | finire **di**, smettere **di** | volere, potere, dovere |
| riuscire **a**, imparare **a** | cercare **di**, provare **di**/**a** | piacere, preferire, sapere |
| continuare **a**, aiutare **a** | decidere **di**, dimenticare **di** | desiderare, amare, osare |
| andare **a**, venire **a** | avere bisogno **di**, avere voglia **di** | |

*Comincio **a** lavorare. · Ho deciso **di** partire. · Voglio partire.* (nessuna preposizione davanti all'infinito dopo un modale o *piacere/preferire*)
